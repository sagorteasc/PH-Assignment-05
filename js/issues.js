        ...
        issueCards.innerHTML = `
            <div class="card bg-base-100 h-full shadow-sm">
                <div class="flex flex-col gap-0.5 p-6 flex-grow">
                    ...
                    <div class="text-[#64748B] text-[12px] flex justify-between items-center">
                        <h3 class="">#${issue.id} ${issue.author}</h3>
                        <p class="">${new Date(issue.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div class="text-[#64748B] text-[12px] flex justify-between items-center">
                        <h3 class="">Assignee: ${issue.assignee === '' ? 'No Data Found' : issue.assignee}</h3>
                        <p class="">${new Date(issue.updatedAt).toLocaleDateString()}</p>
                    </div>
                </div>
            </div>
        `
...
// labels array
const labels = (items) => {
    const label = items.map(item => `<span class="px-1.5 py-2 bg-[#FFF8DB] text-[#D97706] rounded-[100px] font-medium text-[12px]">${item.toUpperCase()}</span>`)
    return (label.join(" "));
}
